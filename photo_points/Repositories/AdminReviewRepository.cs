...
namespace photo_points.Repositories
{
    public class AdminReviewRepository : IAdminReviewRepository
    {
        ...
        {
            var capture = _dbcontext.Captures.Include(c => c.User).Include(c => c.Datas).Include(c => c.PhotoPoint).Include(c => c.Tags).FirstOrDefault(p => p.CaptureId == captureId);
            return capture;
        }

        public IEnumerable<Capture> GetCaptures()
        {
            var captures = _dbcontext.Captures.Include(c => c.User).Include(c => c.Datas).Include(c => c.Tags).Include(c => c.PhotoPoint);
            return captures;
        }
